...
(function (root, factory) {
  ...
  function createSessionManager(deps) {
    ...
    async function doRefresh() {
      try {
        ...
        for (let attempt = 0; attempt < MAX_REFRESH_ATTEMPTS; attempt++) {
          ...
          if (res.ok) {
            // A 200 whose body didn't parse as JSON, or parsed to something
            // that isn't a plain object (a TLS-inspecting proxy's block page,
            // a CDN maintenance page, an SPA index.html fallback), says
            // nothing about the refresh token at all. It is exactly the
            // "proxy error page" case classifyRefreshFailure already treats
            // as transient — but that classifier is never reached on a 2xx,
            // so the same call has to be made here. Leave storage untouched.
            if (
              !res.body ||
            ...
            // readAuthPayload can't distinguish "login against a pre-cutover
            // backend" (where a missing refreshToken legitimately means
            // legacy) from "refresh response dropped the field" — only the
            // caller knows which record it started from. `current` already
            // had a rotating refresh token, so a 200 that comes back without
            // one is never an honest downgrade; treat it as an unusable
            // response and keep the still-valid tokens in place instead of
            // silently demoting the session to legacy.
            if (current.refreshToken && !next.refreshToken) {
              return 'unavailable';
            }
    ...
    /**
     * A `Headers` instance (or any other Map-like object) has no own-
     * enumerable keys, so Object.assign silently copies nothing from it. A
     * plain object is returned as-is; anything iterable via forEach/entries
     * is flattened into one so its values survive the header merge instead
     * of vanishing without an error.
     */
    ...
    async function apiFetch(path, init) {
      const deviceUuid = await getDeviceUuid();
      const extraHeaders = toPlainHeaders(init && init.headers);

      const send = async (record) => {
        const res = await fetchImpl(
          apiBase + path,
          Object.assign({}, init, {
            // Applied last, after the caller's init, so a caller-supplied
            // `credentials` can never defeat the cookie suppression this
            // function exists to provide.
            credentials: 'omit',
            headers: buildHeaders(record, deviceUuid, extraHeaders),
          }),
        );
        return {
          ok: res.ok,
      ...
      if (core.isAccessTokenExpired(record, now()) && record.refreshToken) {
        ...
        // 'unavailable' deliberately falls through and sends the request
        // anyway. Our clock said the token had expired, but the clock is not
        // the authority: if it was wrong the request simply succeeds, and if it
        // was right the 401 below handles it at the cost of one round trip.
        // Returning here instead would let a skewed clock — or a refresh
        // cooldown left over from someone else's rate limit — decide auth.
      }

      const attemptedToken = record.accessToken;
      ...
      // This 401 may be stale: another in-flight apiFetch can have already
      // rotated the token while this request was on the wire, in which case
      // the stored access token has already moved on from the one that just
      // failed. Spending a second rotation on that would turn one expiry
      // into N sequential hits on a per-IP rate-limited endpoint, so a
      // refresh is only warranted when the token that failed is still the
      // one on record; otherwise the fresh token already sitting in storage
      // is the single retry.
      const currentBeforeRefresh = await getAuthRecord();
      let refreshed;
      if (currentBeforeRefresh && currentBeforeRefresh.accessToken !== attemptedToken) {
        refreshed = currentBeforeRefresh;
      } else {
        const outcome = await refreshTokens();
        if (outcome === 'signout') {
    ...
    async function login(credentials) {
      ...
      // `otp` must always be a string, because JSON.stringify DROPS keys whose
      // value is undefined — and the backend reads the key's absence as an
      // opt-in to PassAuth push approval, not as a blank code. On an account
      // with a paired phone, a dropped key returns 200 with `needsApproval`
      // and no token: the phone rings and the popup reports "no secure token
      // received from server", pointing nowhere near the cause. The popup
      // always sends a trimmed string today, so this is a guard against the
      // credentials object arriving without one, not a live bug.
      const otp = typeof c.otp === 'string' ? c.otp : '';

      let res;
      try {
        res = await postJson(
          '/auth/login',
          { email: c.email, password: c.password, otp },
          null,
          deviceUuid,
        );
      } catch {
